...
for each row
begin
dbms_output.put_line('operation reussie pour emp '||:new.nom);
end;
/
...
then dbms_output.put_line('suppression reussie pour emp : '||:old.nom);
end if;
end;
/
...
update emp set salaire = -5 where num = 27546;
...
for C_t in C
loop
dbms_output.put_line(' nom du commercial : '||C_t.nom||' num du commercial : '||C_t.num);
end loop;
end;
/
...
--test
insert into emp (nom,num,salaire,n_dept) values ('zoe',11111,5000,10);
